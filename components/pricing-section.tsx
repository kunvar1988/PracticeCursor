            ...
            <Card key={plan.name} className="bg-white border border-gray-200 shadow-sm">
              <CardHeader className="pb-6">
                <div className="flex flex-col gap-1">
                  <h3 className="text-3xl font-bold text-black">{plan.name}</h3>
                  <p className="text-base text-gray-600">{plan.subtitle}</p>
                </div>
                <div className="mt-6">
                  <span className="text-5xl font-bold text-black">{plan.price}</span>
                  {plan.price !== "Custom" && <span className="text-gray-600 text-base ml-1">per month</span>}
                  {plan.priceSubtext && <div className="text-base text-gray-600 mt-1">{plan.priceSubtext}</div>}
                </div>
              </CardHeader>
